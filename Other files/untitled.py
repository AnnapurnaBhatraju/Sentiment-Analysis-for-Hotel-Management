...
st.header("Enter Feedback and Preferences")
email = st.text_input("Enter your email:")
feedback = st.text_area("Enter your feedback:")
if st.button("Submit Feedback"):
    if email and feedback:
        sentiment = analyze_sentiment(feedback)
        st.write(f"Feedback sentiment: {sentiment}")
        if sentiment == "negative":
            st.error("We apologize for the inconvenience caused.")
        else:
            st.success("Thank you for your positive feedback!")

        st.subheader("Select Preferences")
        preferences = st.multiselect("Choose your preferences", options=recommendations.keys())
        if preferences:
            st.header("Recommendations for You")
            for preference in preferences:
                st.subheader(preference)
                for rec in recommendations[preference]:
                    st.write(f"- {rec}")
            st.success("Have a great stay!")
        else:
            st.warning("Please select at least one preference to get recommendations.")
    else:
        st.error("Please provide both email and feedback.")
"""
